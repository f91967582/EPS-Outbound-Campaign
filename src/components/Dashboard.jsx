...
function Dashboard() {
  ...
  // --- Efectividad Cobro (por Tipificacion) ---
  ...
  const getCount = (item) => Number(item?.total ?? item?.count ?? item?.cantidad ?? 1) || 0;
  ...
  const efectividadCobroPct = totalCalls ? Math.round((cobrados / totalCalls) * 100) : 0;
  ...
  // --- Contactabilidad (por Tipificacion) ---
  const noContactSet = new Set(
    [
      ...
      "Número Ocupado",
      "Posible Contacto",
      "", // When DB value "Tipificacion" comes empty
    ].map(normalize)
  );
  ...
  // --- TOP 5 Tipificacion por Entrantes / Salientes ---
  ...
  const buildTop5Rows = (direction /* "entrante" | "saliente" */) => {
    ...
    <div className="dashboard-container">
      ...
      <TabPanel active={tab === "dashboard"}>
        <section className="kpi-grid">
          ...
          >
            <span className="badge badge-success">{cobrados} Cobrado</span>
            <span className="badge badge-danger">{noCobrados} No Cobrados</span>
          </KpiCard>
          ...
          >
            <span className="badge badge-success">{contactados} Contactados</span>
            <span className="badge badge-danger">{noContactados} No Contactados</span>
          </KpiCard>
          ...
          <KpiCard label="Distribución">
            <div className="dist-row">
              ...
              <span>
                <b>{entrantes}</b> <span className="dist-percent">({entrantesPct}%)</span>
              </span>
            </div>
            <div className="dist-row">
              ...
              <span>
                <b>{salientes}</b> <span className="dist-percent">({salientesPct}%)</span>
              </span>
            </div>
          </KpiCard>
        </section>
      ...
      <TabPanel active={tab === "details"}>
        <DetailsTable data={data} loading={loading} error={error} />
      </TabPanel>
    </div>
  );
}
