...
export default function CampaignsView() {
  ...
  // Logic for other types remains ready for when you need them
  const {
    campaign,
    rows,
    nextToken,
  ...
  const handlePause = async () => {
    ...
    } catch (err) { console.error(err); } finally { setPausing(false); }
  };
  ...
  const handleResume = async () => {
    ...
    } catch (err) { console.error(err); } finally { setResuming(false); }
  };
    ...
    <Box sx={{ bgcolor: alpha(theme.palette.grey[100], 0.4), minHeight: '100vh' }}>
      <Container maxWidth="xl" sx={{ py: 6 }}>
        <Stack direction="row" alignItems="center" spacing={2} mb={5}>
          ...
          <Box>
            <Typography variant="h4" fontWeight={800}>Campañas Outbound</Typography>
            <Typography variant="body2" color="text.secondary">Gestión centralizada de comunicaciones.</Typography>
          </Box>
        </Stack>
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={4}>
            <Stack direction="row" alignItems="center" spacing={1} mb={1.5}>
              ...
              <Chip label={smsCampaigns.length} size="small" variant="outlined" sx={{ height: 18, fontSize: '0.65rem' }} />
            </Stack>
            ...
            {/* Voice and Email sections are now omitted to match your original structure */}
          </Grid>

          <Grid item xs={12} md={8}>
            {!selectedId ? (
              <Box sx={{ height: 400, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', bgcolor: 'background.paper', borderRadius: 4, border: '2px dashed', borderColor: 'divider' }}>
                <Typography color="text.disabled">Selecciona una campaña para ver el detalle.</Typography>
              </Box>
